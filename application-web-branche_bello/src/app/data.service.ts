...
export class DataService {
  ...
  getApidata_selective(ville : string , date : string) : Observable<any> {
    return this.http.get(this.apiUrl_donneclimatique+"/ville/"+ville+"?dateDebut="+date+"&dateFin=2024-12-31");
  }

  // getWeatherByCityAndDate(ville: string, date: string): Observable<any> {
  //   return this.http.get<any>(`https://api.example.com/weather?city=${ville}&date=${date}`);
  // }
  // searchCities(query: string): Observable<any[]> {
  //   return this.http.get<any[]>(`https://api.example.com/cities?search=${query}`);
  // }
}
